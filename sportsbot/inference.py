...
def predict(test_convs,
    ...
    for i, test_conv in enumerate(test_convs):
        tweet_template = test_conv.template + labels_dict[labels[i]]
        ...
        if labels is not None:
            ...
            conversations[str(i)] = [test_conv.template, softmax_tokens, labels[i], top_softmax]
        else:
            softmax_tokens = _label_softmax(predicted_prob, tokenizer, labels_dict)
            conversations[str(i)] = [test_conv.template, softmax_tokens, top_softmax]
    if labels is not None:
        conversations["accuracy"] = str(_calculate_accuracy(labels, answers, labels_dict))
        conversations["validation_loss"] = loss
